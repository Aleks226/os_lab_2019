  ...
  for (int i = 0; i < pnum; i++) {
    ...
    if (child_pid >= 0) {
      ...
      if (child_pid == 0) {
        // child process
        // parallel somehow
        struct MinMax min_max = GetMinMax(array, (array_size*i)/pnum, (array_size*(i+1))/pnum);        // 1
        if (with_files) {
          // use files here	
